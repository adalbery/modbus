...
{   PHoldReg  = hold;  //armazena ponteiro para registradores Holding
    ...
    //Foi acrescido o tempo de  char devido nao ter como saber o momento do in�cio do recebimento
	...
	{	crc ^= msg[pos];				// Faz uma XOR entre o LSByte do CRC com o byte de dados atual
		...
		{	if ((crc & 0b1) != 0)		// Se o LSB for 1:
			{	crc >>= 1;              // Desloca para a direita
				crc ^= 0xA001;          // E faz XOR com o polin�mio 0xA001 (1010 0000 0000 0001 ): x16 + x15 + x2 + 1
			}else						// Sen�o:
			{	crc >>= 1;              // Desloca para a direita
			}
		}
	}	
	return crc;// O formato retornado j� sai invertido (LSByte primeiro que o MSByte)
}
...
{	uint16_t conta;//vari�vel generica para contagem
